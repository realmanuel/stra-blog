...
export default async function BlogPage() {
  ...
  const hero = featuredPost ?? allPosts[0]
  const rest = allPosts.filter((p) => p.slug !== hero?.slug)

  const sellerSpotlight = rest.find((p) => p.category.slug === 'seller-story')

  const gridRow1 = rest.slice(0, 3)
  const widePost = rest[3]
  const tallPost = rest[4]
  ...
  return (
    <>
      ...
      <main>
        ...
        {/* Category filter — client component, gets categories from Sanity */}
